...
### 第三十条：任务级版本强制递增与双向闭环律 (Mandatory Per-Task Version Bump Law)
- **每动必升版 (Every Task Must Bump Version)**：
  - 凡对工程仓库产生实质物理变更的任务（包括新增需求、规则修改、代码重构、配置调整、台账更新），**必须在任务生命周期内对系统实施总版本号执行至少一次显式递增**（遵循 SemVer 规范，默认递增 PATCH 或 MINOR）；
  - 严禁出现“任务已完成并提交 Git，但版本号与上一轮任务完全相同”的悬空提交；纯只读查询交互免于递增。
