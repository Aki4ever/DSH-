...
│   ├── workflow/                    # 流程协同规范
│   │   ├── task_execution_flow.md   # 任务执行六步流程、图形化交互与难度量化打分规范
...
### 3. 任务执行结构化流程与操作指代
遵循 `rules/workflow/task_execution_flow.md` 进行日常任务执行：
- **分类编码与 8 字命名**：`[分类编号][100分制打分] 概述(≤8字)`，如 `[R002][65分] 极简规范与置顶组件`；
- **任务六大分类**：`R (规)` 规则协同、`F (功)` 功能研发、`D (文)` 文档知识、`S (系)` 系统配置、`O (运)` 运维治理、`Q (测)` 质检核验；
- **极简图符与吸顶组件**：采用轻量起止图符（▶ 🚀 / ■ 🏁）与段落吸顶置顶卡片；
- **六步快捷指代**：
  - **【探境】(探)**：环境探知与上下文扫描
  - **【定标】(定)**：意图提炼与边界定标
  - **【筹策】(筹)**：方案推演与步骤拆解
