...
## 💻 四、执行层 CLI 与判定脚本开放接口矩阵 (Open Capability CLI Interfaces)

依据 REQ-067，全域执行层 CLI 与判定工具全面开放标准化调用契约，对内高度封装，对外提供确定性输入、输出与降级预案：

### 1. `cli.control.gates` - 门禁判定与看板内核
- **接口定位**：四道必要性累积门禁（G1骨架/G2结构/G3台账/G4冗余）计算与状态机推进中枢。
- **边界声明**：仅负责推导客观物理状态并产出状态文件；不负责修改或修复业务文件。
- **输入参数**：
  ```bash
  ./scripts/control_gates.sh [check | badge | json | card | reset | graph]
  ```
- **输出回执**：
  - 成功退出码 `0`（门禁全过且正常产出）；失败退出码 `1`（门禁卡点阻断或执行异常）；
  - `status.json` 与控制台 Markdown 看板。
- **安全评级**：`[READONLY / LOW_RISK]`（逃生通道免阻断）。

### 2. `cli.task.naming` - 会话命名规范推断与校验
- **接口定位**：会话首动握手三段式命名规范：`[分类编号][难度分] 8字概述`。
- **边界声明**：仅操作当前会话标题与会话存储；严格校验六大分类与 8 字字数红线。
- **输入参数**：
  ```bash
  ./scripts/name_me.sh "[分类编号][难度分] 概述" | --auto
  ./scripts/check_task_naming.sh [--exit]
  ```
- **输出回执**：状态码 `0`（合规）或 `1`（不合规，打印规范引导提示）。
- **安全评级**：`[LOW_RISK]`。

### 3. `cli.scan.redundancy` - 文本与规则相似度查重
- **接口定位**：基于 Jaccard 词级相似度（阈值 0.85）进行代码与文档实质重复块排查。
- **边界声明**：只读扫描，排除台账与纯表格；不自动修改文件。
- **输入参数**：
  ```bash
  node scripts/redundancy_scan.mjs --root <路径> [--json] [--self-test]
  ```
- **输出回执**：JSON 结构体或单行汇总（高相似对数、实质块数、最高重复行）。
- **安全评级**：`[READONLY]`。

### 4. `cli.scan.conflict` - 跨文档事实与版本冲突排查
- **接口定位**：针对版本号、统计计数、量化指标、能力标识与死链五类冲突的深度排查。
- **边界声明**：只出裁决报告与仲裁建议，严禁自行对冲突进行取舍。
- **输入参数**：
  ```bash
  node scripts/conflict_scan.mjs --root <路径> [--json] [--self-test]
  ```
- **输出回执**：高危冲突数、中危冲突数及详细行号定位。
- **安全评级**：`[READONLY]`。

### 5. `cli.scan.legacy_align` - 存量规范校准与索引覆盖扫描
- **接口定位**：全量枚举磁盘工程资产，核验命名、入口引用、版本归位与索引登记。
- **边界声明**：待对齐项必须在当前任务清零，或登记于 `ai-control/config/legacy_align_exempt.txt`。
- **输入参数**：
  ```bash
  node scripts/legacy_align_scan.mjs --root <路径> [--json] [--self-test]
  ```
- **输出回执**：待对齐项清单（L1命名 / L2入口 / L3版本 / L4索引覆盖）。
- **安全评级**：`[READONLY]`。

### 6. `cli.route.navigate` - 地图式能力导航与快速入口提取
- **接口定位**：意图分析与路径规划，输出五段式地图导航卡片或当前任务交付入口。
- **边界声明**：基于能力索引与快捷通道进行合法路由匹配；不伪造未登记路径。
- **输入参数**：
  ```bash
  node scripts/route_navigate.mjs <目标口令/意图> | --entry
  ```
- **输出回执**：标准五段式 Markdown 地图导航卡片或结构化直达入口链接。
- **安全评级**：`[READONLY]`。
