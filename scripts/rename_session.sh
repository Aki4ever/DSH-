...
# R2/R3 分类编号：单字母 + 三位数字，字母限 R/F/D/S/O/Q
if ! printf '%s' "$CODE" | grep -qE '^[RFDSOQ][0-9]{3}$'; then
  echo "❌ 命名不合规：分类编号必须「单字母+三位数字」，字母限 R/F/D/S/O/Q。" >&2
  echo "   收到： [${CODE}]   正确示例： [R048]" >&2; exit 1
fi

# R4 难度分：1~100 整数且带"分"字
if ! printf '%s' "$SCORE" | grep -qE '^[0-9]{1,3}分$'; then
  echo "❌ 命名不合规：难度分必须是 1~100 的整数且带「分」字（如 [50分]）。" >&2
