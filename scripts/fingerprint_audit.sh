...
MANAGED_DIRS=(
  ...
  "ai-control"
  # ── 2026-09-23 补入 ──────────────────────────────────────────────────────────
  # 为什么补 `memory/`：它是台账与记忆中枢的所在地，此前完全不在受管范围，
  # 连指纹台账自己都没有指纹 —— 台账被改动后没有任何机制能发现。
  # 实测评测：补入后台账多出 6 条 memory 条目（含台账自身）。
  #
  # 为什么不补 `assets/`（重要，避免后人重复踩这个坑）：
  # `assets/` 下的 SVG/PNG 是**自动生成物**（`control_gates.sh graph`、出图通道产出），
  # 每次运行内容都会变。把它们纳入指纹监控会造成**永久性假漂移** ——
  # 新鲜度永远报 Stale，久而久之没人再看这个指标。
  # 该目录本就由下方 collect_files 的排除规则刻意跳过（见那里的说明），
  # 因此这里**不要**把 assets 加进 MANAGED_DIRS：加了也会被同一处规则排除，
  # 徒增"已纳管"的错觉。图形资产的版本一致性由 `legacy_align_scan.mjs` 的
  # assets 扫描负责，那才是对的手段（比对版本语义，而非比对字节指纹）。
  "memory"
)
...
# 辅助函数: 提取文件头部声明的版本号
#
# 口径（2026-09-23 定，解决一处长期语义歧义）：
#   脚本文件同时存在两个不同含义的版本号，**必须分开**，不能混为一谈：
#     · 「脚本自身版本」= 该脚本自己的迭代次数，与工程总版本无关，**不参与新鲜度比对**；
#     · 「对齐工程版本」= 该脚本上一次与工程总版本对齐时的版本，**这才是新鲜度比对的依据**。
#   历史问题：原实现是"前 25 行内第一个含『版本』字样行里的第一个 vX.Y.Z"，
#   于是脚本头部的「脚本自身版本」（如 v2.3.0）被误当作"落后于工程总版本"长期挂红，
#   而它其实只是脚本自己的迭代号 —— 属**假漂移**，会让人对新鲜度指标失去信任。
get_declared_version() {
  local file="$1"
  local ver
  # ⓪ 已退役/已废弃的文件不参与新鲜度比对 —— 它们不再与工程总版本对齐，
  #    却常因头部残留的"测试目标：vX.Y.Z"被误报为"存量落后"。
  #    实测：scripts/test_v180_spec.sh 头部写的是 v1.8.0 的**测试目标**，
  #    并非它自己的版本，却长期挂在 Stale 名单里 —— 典型的假漂移。
  if head -n 25 "$file" 2>/dev/null | grep -qE "已退役|已废弃|DEPRECATED"; then
    echo "-"
    return
  fi
  # ① 优先取"对齐工程版本"（脚本类文件的正确口径）
  ver=$(head -n 25 "$file" 2>/dev/null \
        | grep -E "对齐工程版本|对齐版本" \
        | grep -m1 -oE "v[0-9]+\.[0-9]+\.[0-9]+" || true)
  # ② 其次取受管文档的头部版本声明（文档只有这一个版本号，语义即"对齐总版本"）
  if [ -z "$ver" ]; then
    ver=$(head -n 25 "$file" 2>/dev/null \
          | grep -E "当前(文档|模板|台账|系统实施总)版本|当前文档版本" \
          | grep -m1 -oE "v[0-9]+\.[0-9]+\.[0-9]+" || true)
  fi
  # ③ 兜底：仍按旧口径取第一个版本号（用于未标注类型的文件，保持向后兼容）
  if [ -z "$ver" ]; then
    ver=$(head -n 25 "$file" 2>/dev/null \
          | grep -E "版本|version" \
          | grep -m1 -oE "v[0-9]+\.[0-9]+\.[0-9]+" || true)
