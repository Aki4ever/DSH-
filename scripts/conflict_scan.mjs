 ...
 * 从文本抽出文档头部声明的版本文档号。
 *
 * 采纳范围**限定在文档头部**（前 25 行）：版本声明是元数据，不是叙述内容。
 * 只在全文匹配会误判 —— 实测一篇正文里引用历史版本 "README v2.9.0" 的
 * 分析文档，被当成"该文档自身版本为 v2.9.0"，凭空造出一条版本冲突。
 * 头部没有声明时返回 null（该文件不参与 C1 比对），而不是退而求其次去正文里猜。
 *
 * 兼容两种写法（它们语义相同，必须都能认）：
 *   `**当前文档版本**：`v3.0.0`` 与 `**当前系统实施总版本**：`v3.0.0``
 */
