 ...
 * 头部版本行识别。
 *
 * 踩坑记录（2026-09-23）：原正则只认「当前文档/模板/台账版本」「对应实施版本」「实施版本」，
 * **漏掉了「当前系统实施总版本」**——而 README 与需求文案用的正是这个写法。
 * 后果：总版本推进到 v3.2.0 后，这三份文档头部仍写着 v3.1.0，
 * 被冲突检测器与存量校准器同时判为**高危**（版本孤岛）。
 * 现在把「当前系统实施总版本 / 系统实施总版本」并入可选写法，避免同类陷阱复发。
 */
const VERSION_LINE = /^(\s*>\s*-\s*\*\*(当前(?:文档|模板|台账|系统实施总)版本|系统实施总版本|对应实施版本|实施版本)\*\*：\s*`v)(\d+\.\d+\.\d+)(`.*)$/
