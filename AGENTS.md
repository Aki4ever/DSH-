...
门禁是**累积**的：必须 G0→G1→G2→G3→G4 全部通过才算"可执行"。
...
任务完成或输出汇报时必须满足六大模块（不得遗漏任何一项）：

1. **首屏最显眼处显式结论**：首行必须使用醒目标签明确物理状态（🟡【仅方案规划态 · PLAN ONLY】还是 🟢【真实实施完成态 · DELIVERED】），绝不允许计划与实施混淆；
2. **结构化交付入口**：本次产出/修改的主要文件（写成 Markdown 行内代码）；
3. **量化进度**：运行 `./scripts/control_gates.sh badge` 并把结果贴进收尾；
4. 🌟 **【执行效果】（新增核心模块）**：运行 `./scripts/audit_execution.sh`，由管控审计智能体 (Control Auditor) 输出 0~100 分量化评分卡片与扣分明细；按规则履约打 100 满分，越违规越低分，低于 85 分必须立即整改；
5. **未验证项声明**：凡未经真实验证的部分，必须明说"未验证"，不得含糊；
