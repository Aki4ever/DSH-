...
本工程受 **AI 执行流程管控** 约束。任何改动型动作（写文件、改配置、跑构建）
之前，先运行管控看板确认门禁状态：
...
任何规则的新增、修改、注销，遵循 `rules/workflow/change_flow.md`：

```
接收意图 → 查重拦截 → 登记台账 → 编写规则 → 联动排查 → 提交推送
```

- **查重是硬要求**：先比对 `docs/requirements.md` 与既有规则，杜绝碎片化冗余；
- **台账必须同步**：每次规则变动都要在 `docs/requirements.md` 留痕；
- **冗余自检**：可运行 `node scripts/redundancy_scan.mjs --root .` 检测真冗余。
...
ai-control/  AI 执行流程管控系统（门禁判定 + 硬门禁插件）
assets/      图形资产
```
