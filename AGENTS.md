...
任务完成或输出汇报时必须满足：
1. **文末固定四联装总结（强制收尾结构）**：无论汇报长短，在输出的最末尾处必须以固定四项总结收尾：
   - **当前状态**：明确标注【规划阶段】或【实施阶段】；
   - **核心结论/输出物**：有输出物就给具体输出物，没有输出物就给核心结论；
   - **输出物地址**：有输出物给出物理文件路径（行内代码格式），没有输出物填「无」；
...
3. **全域所有 DSH 工程常态化永久生效**：本管控机制对所有 DSH 工程文件夹永久生效，杜绝任何项目特权或裸跑脱管。
