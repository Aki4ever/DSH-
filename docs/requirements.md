...
### REQ-041: AI 执行流程管控系统（门禁内核 + 常显看板 + 硬门禁插件）
- **当前状态**：`[ACTIVE]` 生效中
- **实施版本**：`v2.8.0`
- **提出时间**：2026-09-22
- **最新更新**：2026-09-22
- **标准需求重构文案 (利于 Agent 执行的标准化任务单)**：
  ...
  - **核心诉求与交付物**：
    1. **门禁内核**：`scripts/control_gates.sh` 以磁盘状态推导 G1~G4 门禁，输出量化看板与 `status.json`，写入 `$DSH_HOME/.dsh-control/`；
...
- **验收标准**：
  ...
  - [x] 硬门禁插件自检 **33/33** 通过（可拒绝、可自救、可绕过、过期失败关闭、冷启动自举、故障安全降级）；
